...
var ProAttrGroup={
    ...
    shoppingCar:function(){
        $("#shoppingCar").bind("click",function(){
            ...
            if(chioceAttrData!=null && chioceAttrData.length<showData.length){
                ...
                alert("选择的套餐是:"+ProAttrGroup.changeValue(showData,chioceAttrData))
            }
        })
    },
    /*
     * 初始化用户选择事件
     */
    initUserChioce:function(){
        //展示产品的所有属性组
        ProAttrGroup.showAttr(showData);
        ProAttrGroup.initSKU();
        $('.sku').each(function () {
            var self = $(this);
...
$(function () {
    //初始化用户选择事件
   ProAttrGroup.initUserChioce();
   //加入购物车
   ProAttrGroup.shoppingCar();
});
