...
contextMenu.prototype.init = function () {
    ...
    if (this.opt.hasTitle) {
        ...
        if (this.opt.menu.title.icon && this.opt.menu.title.icon.trim().length != 0) {
    ...
    if (items) {
        for (i = 0; i < items.length; i++) {
            ...
            if (items[i] === "-") {
                ...
                if (items[i].icon && items[i].icon.trim().length != 0) {
                    var image = this.opt.hasIcon ? "img" : "span";
...
contextMenu.prototype.bind = function () {
    ...
    self.on(self.target, "contextmenu", function (e) {
        self.show(e);
        event.returnValue = false;
    });


    self.on(document, "click", function () {
        self.hide();
    });
